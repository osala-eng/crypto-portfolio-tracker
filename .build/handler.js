...
var dataAccess_1 = require("./dataLayer/dataAccess");
...
var handle = function (event, context) { return __awaiter(void 0, void 0, void 0, function () {
    var user, validEmail, DB, response, e_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                user = JSON.parse(event.body);
                validEmail = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test(user.email);
                if (user.username.length < 5 || !validEmail || user.password.length < 8) {
                    throw new Error('Credentials does not meet required standards');
                }
                DB = new dataAccess_1.DataAccess();
                return [4, DB.createUser(user)];
            case 1:
                response = _a.sent();
                return [2, {
                        statusCode: 200,
            ...
            case 2:
                ...
                return [2, {
                        ...
                        body: JSON.stringify(e_1.message)
                    }];
            case 3: return [2];
        }
    });
